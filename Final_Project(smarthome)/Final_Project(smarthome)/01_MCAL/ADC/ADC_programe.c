...
void ADC_u8StartConversionAsynch (u8 Copy_u8Channel , u16 * Copy_pu16Result , void (*Copy_pvNotificationFunc)(void))
{
	//u8 Local_u8ErrorState = OK ;
	...
	if ((Copy_pu16Result != NULL) && (Copy_pvNotificationFunc != NULL))
	{
		if (ADC_u8State == IDLE)
		{
			...
			SET_BIT(ADSAR , ADSAR_ADIE);					// When Conversion is finished, will execute ISR directly. 
		}
		else
		{
			//Local_u8ErrorState = BUSY_STATE ;
...
void ADC_u8StartChainAsynch (Chain_t * Copy_Chain)
{
	//u8 Local_u8ErrorState = OK ;
...
void __vector_16 (void)
{
	if (ADC_u8ISRState == SINGLE_CHANNEL_ASYNCH)
	{
		...
		ADC_pvNotificationFunc();		// Call back function.
